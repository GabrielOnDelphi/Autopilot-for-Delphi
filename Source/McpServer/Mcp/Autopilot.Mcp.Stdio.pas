...
(*=====================================================
   ...
   The MCP server's stdio main loop. Reads one JSON-RPC line at a time from
   stdin, dispatches it, writes the response (if any) to stdout. Logs every
   request and response to %TEMP%\Autopilot\<exe>-<pid>.log via Bridge.Log.

   Per the MCP spec:
     - stdin / stdout carry JSON-RPC messages, one per line, no embedded
       newlines, UTF-8.
     - stderr is for human-readable logs and MUST NOT be parsed by the client.
     - Shutdown is signalled by the parent closing our stdin (we see EOF on
       Input). No explicit Ctrl-C handling needed.

   We set the console output codepage to UTF-8 once at startup as a defensive
   measure — Writeln(Output, ...) then emits raw UTF-8 bytes regardless of the
   user's chcp. JSON itself is ASCII-safe so this matters only when a tool's
   response contains non-ASCII (e.g. a control's Caption with accented chars).
=====================================================*)
